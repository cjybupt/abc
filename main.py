...
# from STAGNN.Code.load_data import load_Data
from conf import *
...
dis_matrix = load_data.get_distance_matrix(path_matrix_dis)
...
def train(epoch,la):
    ...
    for train_node_feature in train_iterator:
        ...
        train_lab = [data[-1][0].x for data in train_node_feature]  # 每个图序列中最后一个时间步的节点特征作为标签

        optimizer.zero_grad()
        train_lab = torch.stack(train_lab)  # 标签张量堆叠成一个批次张量
        pre_lab = model(batch_size, n_graph,n_vertex, train_node_feature, dis_matrix)  # todo
...
def val(la):
    ...
    for  val_node_feature in val_iterator:
        ...
        pre_lab = model(batch_size, n_graph,n_vertex, val_node_feature, dis_matrix)
...
def tst(la):
    ...
    for  tst_node_feature in tst_iterator:
        ...
        pre_lab = model(batch_size, n_graph,n_vertex, tst_node_feature, dis_matrix)
...
def run(total_epoch,la):
    ...
    checkpoint = "/root/autodl-tmp/model_weights_divvy_30min_aggr_geo.pth"  # note
